...
function callbackSetBid() {//接收投标对象的信息，发布日期， 截止日期，报价状态，竞标人数，招标名称以及是否已报价和报价金额
    if (this.readyState == 4 && this.status == 200) {
        ...
        if (json.result) {
            ...
            var finishAlready = json.finishAlready;
            ...
            if (registAlready) {//若已报价，则加载投标信息
                bidPost = true;
                bidInfoShow();
                refreshBidContent();
                var bidAmountMe = document.getElementById("bidAmountMe");
                bidAmountMe.innerText = json.amount;
                document.getElementById("bidFormTablePrepareButtonID").style.display = "block";
                document.getElementById("bidFormTableResetButtonID").style.display = "block";
            }
            if (finishAlready) {//若已结束
...
function resetBidderAmount() {//重置投标内容中投标金额的值
    ...
    var amountInput = document.getElementById("bidderAmountInput");
    var amountButton = document.getElementById("bidderAmountButton");
    amountInput.innerHTML = '<input type="number" class="bidFormTableInput" id="bidderAmount" max="4294967295" min="1" value=' + amountInputValue + '></input>';
...
function createNewBid() {//创建新的招标
    ...
    if (name != "" && counts != "" && dateStart != "" && dateEnd != "" && bidName != "" && counts >= 2 && counts < 4294967295) {
